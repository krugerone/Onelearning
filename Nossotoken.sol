...
contract owned {
    ...
    modifier onlyOwner {
        require(msg.sender == owner, "Need be owner");
        _;
    }
...
contract NossoToken is owned {
    ...
    constructor (uint256 initialSupply, string memory tokenName, string memory tokenSymbol, uint8 decimalsUnits) public { 
    ...
    function transfer(address _to, uint256 _value) public {
        ...
        require (balancesOf[msg.sender] >= _value, "No balance");
        // if (balancesOf[msg.sender] < _value) revert();           // Check if the sender has enough

        require (balancesOf[_to] + _value > balancesOf[_to], "Invalid balance");
